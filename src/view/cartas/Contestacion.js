import { Box, Link, Center, FormControl, Input, ScrollView, VStack, Pressable, Avatar, Text, useToast, Button, Icon, View, HStack, CheckIcon, Heading } from 'native-base'
...
export default function Contestacion({ navigation, route }) {
    ...
    const cargarDatosGuardados = async () => {
        try {
          const savedData = await AsyncStorage.getItem("formDataContestacion");
          if (savedData) {
    ...
    const handleInputChange = (setter, value) => {
        setter(value);
        AsyncStorage.setItem("formDataContestacion", JSON.stringify({
            nombre_patrocinador,
            agradezco_por,
            te_cuento_que,
    ...
    const acceder = async () => {
        if (validar()) {
            ...
            try {
                const res = await ReactNativeBlobUtil.fetch('POST', API_URL + "responder-carta-contestacion", {
                ...
                if (result.success) {
                    await AsyncStorage.removeItem("formDataContestacion");
                    ...
                    <ScrollView contentContainerStyle={{ flexGrow: 1 }} bg="white">
                        {
                            ...
                            >
                                <Heading size={'sm'}>DE PARTE DE TU PATROCINADOR</Heading>
                                <Pdf
                                    source={{ uri:urlCarta }}
                        ...
                        <Box mx={3}  minWidth="95%">
                            <Heading>Complete información</Heading>
                            <FormControl >
                            ...
                            <FormControl >
                                <FormControl.Label>Agradesco por la</FormControl.Label>
                                <Input onChangeText={(value) => handleInputChange(setagradezco_por, value)} value={agradezco_por} placeholder="" />
                            </FormControl>
